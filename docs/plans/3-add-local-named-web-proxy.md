...
- 2026-06-08 (implementation): Mina's `--global` project picker was empty even though the UI loaded. `mina web --global` lists projects by shelling out to the `mori` binary (`Mori.runWith defaultMoriBinary` in `mina-cli/src/Mina/CLI/Web.hs`), and `mori` reads the registry from PostgreSQL via `MORI_PG_CONNECTION_STRING`. The `mina-web` launchd agent inherited neither that variable (set for interactive shells via `programs.zsh.sessionVariables` in `home/mori.nix`) nor a PATH containing `mori`, so every `/api/mori/*` endpoint returned 503. Reproduced exactly with `env -u MORI_PG_CONNECTION_STRING mori registry list` → "MORI_PG_CONNECTION_STRING environment variable not set". Fixed by exporting the same conn string and prepending `${pkgs.mori}/bin` to PATH in the wrapper (and setting `EnvironmentVariables` on the agent), mirroring the `mori-automate` agent in `home/mori.nix`. General lesson for these agents: a launchd service that shells out to a sibling CLI inherits neither session variables nor the Nix-profile PATH — both must be set explicitly.
- 2026-06-08 (implementation): Same class of issue as the empty project picker, but for `rei`: Mina's intention cards showed `ReiBinaryUnavailable {binary = "rei", message = "rei: createProcess: posix_spawnp: does not exist"}`. `mina web --global` shells out to `rei intention show <iid> --json` (`Mina.Rei.Tree`); under launchd, `rei` was not on PATH, and even once found it needs `REI_PG_CONNECTION_STRING` plus the keiro routing var `REI_KIROKU_CONTEXTS` (message-db is frozen post-EP-24, so without it `rei` reads stale/missing data). Fixed by extracting the shared rei CLI environment into `home/rei-cli-env.nix` (single source of truth for the conn string + contexts list, consumed by both `home/rei.nix` and `home/mina.nix`) and having the `mina-web` wrapper prepend `rei` to PATH and export that env. Verified by running `rei intention show <iid> --json` under a clean `env -i` shell with only the wrapper's variables — the intention resolved. This generalizes the prior lesson: an agent that drives a UI by spawning sibling CLIs must reproduce, for every such CLI, both its PATH entry and its full runtime environment.
